...
let returnBook = async (req, res, next) => {
    try {
        ...
        if (borrowRecord.status == "borrowed") {
            ...
            let status = "returned";
            let updateBorrowRecord = await Borrow.findByIdAndUpdate(borrowId, { returnDate, status }, { new: true });
...
const userBorrows = async (req, res, next) => {
    try {
...
const allBorrows = async (req, res, next) => {
    try {
